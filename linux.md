...
* **`sync`**：为了加快对磁盘文件的读写速度，位于内存中的文件数据不会立即同步到磁盘上，因此关机之前需要先进行 sync 同步操作，即使**`shutdown`/`reboot`/`halt`**等指令在关机前已经进行了sync的调用
...
* **`chown`**：change owner的缩写，改变文件拥有者
  * **`chown [账号名] [文件]`**：改变所属群组
  * **`chown -R [账号名] [文件]`**：进行递归的持续变更，即同次目录下的所有文件、目录都更新成为这个群组。常常用在变更某一目录内所有的文件的情况。
