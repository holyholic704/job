...
**随着服务器写请求的增多，AOF 文件会越来越大，体积不断增大的AOF文件很可能会用完硬盘空间，而且还原操作执行时间可能会非常长**。例如对一个计数器调用了 100 次 incr，为了保存这个计数器的当前值， AOF 文件就需要使用 100 条记录。然而在实际上，只使用一条 set 命令就足以保存计数器的当前值了，其余 99 条记录实际上都是多余的
